...
void System :: move(int i){ // Propose a MC move for particle i
  if(_sim_type == 3){ //Gibbs sampler for Ising
    double p = 1.0/(1+exp(-2*_beta*(_J*(_particle(this->pbc(i-1)).getspin()+_particle(this->pbc(i+1)).getspin())+_H))); // probabilità che lo spin sia +1
    if (_rnd.Rannyu() < p){
  ...
  if (input.is_open()){
    while ( !input.eof() ){
      ...
      if( property == "POTENTIAL_ENERGY" ){
        ...
        index_property++;
        _vtail = (8.0*M_PI*_rho)/(3.0*pow(_r_cut,3))*(1.0/(3.0*pow(_r_cut,6))-1);
        ...
        index_property++;
        _ptail = (32.0*M_PI*_rho)/(3.0*pow(_r_cut,3))*(1.0/(3.0*pow(_r_cut,6))-1.0/2.0);
      } else if( property == "GOFR" ){
        ofstream coutgr("../OUTPUT/gofr.dat");
        //coutgr << "# DISTANCE:     AVE_GOFR:        ERROR:" << endl;
        coutgr << "#  DISTANCE:   ACTUAL_GOFR:   AVE_GOFR:      ERROR:" << endl;
        coutgr.close();
        input>>_n_bins;
        _nprop+=_n_bins;
        ...
        input>>_n_bins_v;
        _nprop += _n_bins_v;
        _bin_size_v = 4.0*sqrt(_temp)/(double)_n_bins_v; // uso radice della temperatura
        _measure_pofv = true;
...
void System :: read_configuration(){
  ...
  if(cinf.is_open()){
    ...
    for(int i=0; i<_npart; i++){
      ...
      _particle(i).setposition(0, this->pbc(_side(0)*x, 0)); // moltiplicare per 0.5 esercizio 4.2
...
void System :: measure(){ // Measure properties
    ...
    for (int i=0; i<_npart-1; i++){
      for (int j=i+1; j<_npart; j++){
        ...
        if (_measure_gofr){
          ...
          if (bin < _n_bins){
            _measurement(_index_gofr + bin) += 2.0/(_rho*_npart*4./3.*M_PI*(pow(bin*_bin_size+_bin_size,3)-pow(bin*_bin_size,3)));
          }
        }
        if(dr < _r_cut){
  ...
  if (_measure_pofv){
    for (int i = 0; i < _npart; i++){
      double velocity = sqrt(dot(_particle[i].getvelocity(),_particle[i].getvelocity()));
      if (velocity < _n_bins_v*_bin_size_v){
        bin = int(velocity/_bin_size_v);
        counts++;
        bin_counts[bin]++;
      }
    }
    double norm = 1.0/(double(counts)*_bin_size_v);
  ...
  if (_measure_pressure){
    if (_sim_type == 1) kenergy_temp = 3.0/2.0 * _temp;
  ...
  if (_measure_chi){
    ...
    chi_temp = pow(chi_temp,2);
    _measurement(_index_chi) = _beta * chi_temp;
  }
...
void System :: averages(int blk){
  ...
  if (_measure_cv){
    _average(_index_cv) = pow(_beta,2)*(_average(_index_cv)-pow(_average(_index_tenergy)*_npart,2));
  }
  ...
  if (_measure_pofv){
    ...
    for (int i = 0; i < _n_bins_v; i++){
      double velocity = (i + 0.5) * _bin_size_v;
